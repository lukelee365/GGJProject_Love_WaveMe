using UnityEngine;
using System.Collections;

public class Lightning : MonoBehaviour {

	private LineRenderer lineRenderer;
	private float maxZ = 0f;
	private int numOfSegments = 2;
	private Color colour = Color.white;
	private float PosRange = 0f;
	private Vector2 midPoint;
	public Vector3 origin;
	public Vector3 dest;

	
//	void Start () {
//		lineRenderer = GetComponent<LineRenderer>();
//		lineRenderer.SetVertexCount(numOfSegments);
//		for(int i=1; i < numOfSegments-1; i++)
//		{
//			float z =((float)i)*(maxZ)/(float)(numOfSegments-1);
//			lineRenderer.SetPosition(i, new Vector3(Random.Range(PosRange,-PosRange),Random.Range(PosRange,-PosRange),z));
//		}
//		 lineRenderer.SetPosition(0, new Vector3(0f,0f,0f));
//		lineRenderer.SetPosition(numOfSegments-1, new Vector3(0f,0f,0f));
	//}

	void Start () { //With quadratic mid point calc
		lineRenderer = GetComponent<LineRenderer>();
//		lineRenderer.SetVertexCount(numOfSegments);
//
//
//		midPoint = new Vector2(Random.Range(origin.x,dest.x),Random.Range(origin.y,dest.y));
//
//		for(int i=1; i < numOfSegments-1; i++)
//		{
//			float z =((float)i)*(maxZ)/(float)(numOfSegments-1);
//
//			float x = -midPoint.x*z*z/16f + z*midPoint.x/2f;
//
//			float y = -midPoint.x*z*z/16f + z*midPoint.x/2f;
//
//			lineRenderer.SetPosition(i, new Vector3(Random.Range(PosRange,-PosRange),Random.Range(PosRange,-PosRange),z));
//		}
//		lineRenderer.SetPosition(0, origin);
//		lineRenderer.SetPosition(numOfSegments-1, dest);
	}




	void Update () {
		colour.a -= 10f*Time.deltaTime;

		lineRenderer.SetColors(colour,colour);
		if(colour.a <= 0f)
		{
			Destroy(this.gameObject);
		}
	}
}